...
do
	...
	get=$(curl -ksu "$username":"$password" -H "content-type: text/xml" "$server"/JSSResource/computers/id/"$id" | xmllint --xpath '/computer/general/remote_management/managed/text()' - )
	echo "ID $id is Managed: $get"
done